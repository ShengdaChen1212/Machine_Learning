...
def get_21(origindata, lead, Fiducial_21, option=True):
    feature_list = []
    stop_cnt  = 0
...
def feature_gen(Fiducial_21, lead, option=True):
    if option:
        ...
        feature = get_21(dataset, lead, Fiducial_21, option=False)
    return feature
...
def to_csv(feature, option=True):
    if option:
        print('='*30)
        feature.to_csv('training_features_noexpand.csv', index=False)
        print('Generating Training CSV DONE!')
    else:
        print('='*30)
        feature.to_csv('testing_features.csv', index=False)
